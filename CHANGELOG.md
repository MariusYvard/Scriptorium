...
Le plugin devenait correct sur un document anglais depuis la version 0.13.0 ; il devient utilisable par un auteur anglophone. Trois lots câblent les vingt-sept scripts sur une couche de libellés commune, un quatrième étend les déclencheurs des compétences et des agents à l'anglais, un cinquième classe les vingt-six playbooks de genre face au bilinguisme.

### Added
- Couche d'affichage bilingue : nouveau module scripts/libelles.py, une clé plate à espace de noms par script (`scorecard.entete`, `traceability.titre`), chaque clé portant le couple français-anglais et un formatage à paramètres nommés (l'ordre des mots change d'une langue à l'autre, un format positionnel se casserait à la première inversion). `t(cle, langue, **params)` rend un libellé, `valeur(espace, machine, langue)` traduit l'affichage d'une valeur machine sans la modifier, `motif(motif_fr, langue)` relit un motif de mesure non faite porté par les données. Une clé absente d'une langue retombe sur le français et le déclare (préfixe de repli) plutôt que de rendre une traduction silencieusement incomplète. 812 clés.
- Le choix de conception qui porte toute la version : la valeur machine ne change jamais de langue. Les verdicts fermés, les décisions éditoriales, les noms de règle et de sévérité, les noms des cinq axes du scorecard et toutes les clés JSON restent les chaînes françaises actuelles dans les deux langues d'affichage, en structure de données comme en sortie `--format json`. La raison n'est pas la paresse : `emprunts.py` branche sur les verdicts de `check-droits.py`, des modules de `evals/cas/` les comparent littéralement et `tools/gold.py` les confronte aux étiquettes gelées de `evals/gold/*/manifeste.json`. Traduire ces chaînes en place aurait cassé cette chaîne de dépendances et le jeu d'or. Seul le libellé affiché change de langue ; une valeur ajoutée au code sans entrée dans la table `VALEURS` ressort marquée plutôt que traduite en silence.
- Les vingt-sept scripts gagnent l'option `--langue-affichage fr|en`, résolue une seule fois par `resoudre_affichage()`. Deux familles. Treize scripts qui mesurent un document (lint-style, scorecard, readability, traceability, verify-sources, audit-doc, ai-fingerprint, check-disponibilite, check-temporel, coherence, numbers, plan-check, terminology) prennent par défaut la langue d'analyse déjà résolue : le pragme `lint-style:langue=en` posé dans le document suffit, sans option supplémentaire. Treize scripts qui portent sur un objet sans pragme de langue possible (un registre JSON, un binaire bureautique, un PDF, un fichier `.bib`, un dossier d'images, une charte, un projet) prennent par défaut le français et le déclarent dans leur propre aide. `figures.py` tient les deux langues sans les confondre : `--langue` est la langue de dessin, celle des étiquettes tracées dans le SVG livré, `--langue-affichage` est celle du regard critique de `--audit` au terminal et prend par défaut la langue de dessin plutôt que la langue d'analyse, faute de document à analyser.
- Ce qui reste enregistré n'est pas de l'affichage. Le journal de `projet.json`, le catalogue d'`images.py` et l'inventaire de `gabarit.py` gardent leurs chaînes françaises, quelle que soit la langue d'affichage demandée : ce sont des données écrites sur le disque et relues plus tard, par une autre commande ou une autre session, le hash de continuité d'une frontière de `projet.json` étant calculé sur la sérialisation du journal. Un projet commencé en français et repris en anglais se serait contredit à la relecture.
- Compétences et agents bilingues : les quatre `SKILL.md` et les cinq agents (`contradicteur`, `controle-qualite`, `redacteur`, `synthese-sources`, `verificateur-faits`) portent des déclencheurs anglais à côté des français, en vocabulaire d'usage réel, plus un exemple anglais par agent. Règle de langue de travail écrite dans les quatre routeurs : la langue de la conversation ne détermine pas la langue du document, elle se fixe au cadrage et se propage par le pragme et par `projet.json` ; `atelier`, `controler` et `produire` posent une question fermée quand rien n'est déclaré plutôt que de retomber en silence sur le français, `livrer` hérite de la langue du document validé sauf déclinaison explicitement demandée dans une autre langue.
- Playbooks de genre classés face au bilinguisme : nouvelle référence transverse `skills/produire/references/genres-anglais.md`, chargée en plus du playbook du genre pour un document anglais et jamais à sa place, sur le modèle déjà en place du couple `style.md` et `style-anglais.md`. Les vingt-six genres se répartissent en treize universels (la structure ne bouge pas), onze variantes (le genre existe des deux côtés avec des attentes différentes) et deux propres à une langue sans équivalent transposable (dissertation, conclusions et mémoire contentieux) : ces deux derniers le disent dans leur propre playbook, pas seulement dans le fichier transverse, pour qu'un lecteur qui ne charge pas la référence anglaise lise quand même l'absence de correspondant.
- Evals : 756 cas (523 auparavant). Cinq nouveaux modules : `evals/cas/affichage.py`, `affichage-controles.py` et `affichage-outils.py` couvrent le câblage des vingt-sept scripts en trois lots de six, onze et dix, chacun figeant des empreintes de sortie française relevées avant câblage pour attraper une dérive ; `declencheurs.py` verrouille l'état bilingue des neuf descriptions de compétence et d'agent avec un plafond de longueur ; `genres-bilingues.py` verrouille le classement des vingt-six genres et la cohérence des renvois vers `genres-anglais.md`.

### Changed
- `scriptorium/scripts/README.md` : compte de scripts clarifié (vingt-sept outils, `libelles.py` est le vingt-huitième fichier Python du dossier mais une infrastructure interne sans ligne de commande propre, importée par les vingt-sept), section dédiée à `libelles.py`, option `--langue-affichage` documentée dans chaque section concernée.
- `skills/produire/references/langue.md` réécrit : l'ancienne version affirmait que six scripts seulement portaient un rapport bilingue et que les vingt-six playbooks de genre étaient monolingues sans exception ; les deux affirmations sont devenues fausses avec ce lot et le fichier documente l'état réel.

### Fixed
- `audit-doc.py --format text` levait une exception dès qu'un axe du scorecard n'était pas évalué (formatage d'un score `None` avec un gabarit numérique). Le script n'appelait par ailleurs ni `ai-fingerprint.py` ni `coherence.py` avec la langue résolue : sur un document anglais, ces deux mesures restaient cherchées avec les motifs français. Les deux constats sont corrigés ; les trois mesures que le rapport consolide sans les faire passer par la couche de libellés (empreinte IA, cohérence, tableaux) sont désormais nommées dans le rapport anglais plutôt que de laisser croire à une traduction complète.
