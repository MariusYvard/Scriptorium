...
1. Style maison sur tout texte du plugin (fichiers .md des compétences, références, agents) : registre encyclopédique, zéro tiret cadratin, pas de virgule d'Oxford, guillemets droits, lexique promotionnel banni. Le linter fait foi : `python3 scripts/lint-style.py FICHIER`.
...
4. Les evals passent au vert avant toute proposition : `python3 evals/run-evals.py` (211 cas, hors ligne). Un mécanisme déterministe nouveau arrive avec son cas d'eval ; un nouveau lot de cas se range dans un module de `evals/cas/` plutôt que d'allonger le fichier unique.
...
La publication est pilotée par tag : bump de version dans `.claude-plugin/plugin.json` et `.claude-plugin/marketplace.json`, section datée dans `CHANGELOG.md`, tag `vX.Y.Z`, push. La CI vérifie la cohérence, rejoue les evals, construit le `.plugin` et crée la Release. Détail dans `docs/RELEASE.md`.
