...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-0.8.0-1f6feb" alt="version 0.8.0">
...
Scriptorium est un plugin Claude (Cowork et Claude Code) qui transforme une demande de rédaction en document fini. Il applique une méthodologie d'ingénierie textuelle, déplace la rigueur vérifiable du jugement du modèle vers vingt scripts déterministes, et impose un style maison à directives strictes. Quatre compétences couvrent le cycle de vie complet, du cadrage à la livraison, sur vingt-six genres adossés chacun à des sources faisant autorité.
...
Télécharger `scriptorium-0.8.0.plugin` depuis la [page des releases](https://github.com/MariusYvard/Scriptorium/releases), puis l'installer.
...
| `produire` | genre · sourcer · revue-litterature · veille · figure · tableau · equation · style · charte · image | Produit le contenu et fixe la forme. Rédige les vingt-six genres (poster scientifique compris), trouve, pondère et triangule les sources, met en place une veille documentaire, génère figures et tableaux, pose les équations, applique style et charte, extrait les images d'un document source. |
...
| `livrer` | document · decliner | Met en forme le livrable (Word, PDF, HTML, LaTeX charté) et le décline par canal (présentation, poster, résumé, résumé bilingue FR/EN, abstract, post, communiqué). |
...
Vingt scripts en Python pur déplacent la rigueur du jugement du modèle vers un contrôle mécanique et reproductible. Une porte d'intégration continue (`tools/check.py`) verrouille un document contre un seuil de note, et passer outre exige une justification à friction croissante, journalisée.
...
| `verify-sources.py` | URL à nettoyer, doublons, DOI douteux, palier de source par domaine ; en option réseau, triangulation Crossref, OpenAlex et Semantic Scholar avec verdicts gradués et signaux de contamination. |
| `citations.py` | Cinq formats de bibliographie (APA 7, Vancouver, Chicago, MLA, IEEE), bascule entre formats, ancres par citation, validation de champs par type, résolution DOI, PMID et arXiv, dédoublonnage. |
...
| `scorecard.py` | Note déterministe de 0 à 100 sur cinq axes, calcul montré, barres ASCII, plancher par axe, poids externes, seuils par type de document, décision éditoriale, trajectoire et arrêt anticipé. |
...
| `check-presentation.py` | Deck PDF : pages par durée annoncée, densité de texte, backends optionnels en cascade, jamais de mesure inventée. |
| `theme.py` | Validation de charte, contraste WCAG, palettes daltonisme-sûres (Okabe-Ito, Wong), avertissement dichromate, émission du CSS et du préambule LaTeX. |
...
- Communication : livre blanc, discours et allocution, présentation (soutenance), pitch (commercial et levée de fonds), poster scientifique.
...
Le format de travail est le Markdown. La finalisation produit un document Word (`.docx`), un PDF, un HTML autonome dont le CSS dérive de la charte graphique (jetons, focus visible, feuille d'impression, source idéale pour un PDF fidèle), un document LaTeX charté (gabarit de rapport à encadrés sémantiques et macros statistiques, gabarit de poster a0/a1, préambule émis par `theme.py --format latex`), ou une présentation PowerPoint. Les figures sortent en SVG, les tableaux en Markdown, les équations en PDF via LaTeX, et les images d'un document source sont extraites puis replacées, numérotées et à la charte. Un document destiné à une publication peut porter ses déclarations de contribution (CRediT), de financement et d'usage de l'IA selon la politique du support visé.
