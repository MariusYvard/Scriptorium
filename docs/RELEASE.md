...
- 0.6.0 : consolidation de dix-neuf compétences en quatre à sous-commandes (`atelier` : piloter, cadrer, projet ; `produire` : genre, sourcer, revue-litterature, figure, tableau, equation, style, charte ; `controler` : revue, contredire, consensus, humaniser, audit, relecteurs ; `livrer` : document, decliner), sortie HTML autonome pilotée par la charte graphique (`theme.py --format css`), profil de discipline analyse géopolitique, plugin ouvert au chercheur, à l'ingénieur et à l'analyste géopolitique, extraction et placement d'images (`images.py`, `produire` image), evals portés à 37 cas.
