...
version: "1.0"
author: Scriptorium
tags: [quality-control, review, style-guide, compliance, scorecard]
---
...
Charge `skills/controler/references/grille-revue.md` et `skills/produire/references/directives-strictes.md`. Applique trois batteries, chacune en liste de contrôle.

Avant tout, lance la pré-passe déterministe :
```
python3 ${SCRIPTORIUM_ROOT}/scripts/lint-style.py FICHIER
python3 ${SCRIPTORIUM_ROOT}/scripts/verify-sources.py FICHIER
python3 ${SCRIPTORIUM_ROOT}/scripts/traceability.py FICHIER
python3 ${SCRIPTORIUM_ROOT}/scripts/terminology.py FICHIER
python3 ${SCRIPTORIUM_ROOT}/scripts/numbers.py FICHIER
python3 ${SCRIPTORIUM_ROOT}/scripts/scorecard.py FICHIER
python3 ${SCRIPTORIUM_ROOT}/scripts/audit-doc.py FICHIER
```
...
## Lentilles et biais à s'auto-appliquer

Applique à ta propre relecture les trois lentilles et les pièges recensés dans `references/biais-relecteur.md` : validité interne (l'argument s'effondre-t-il si une preuve citée est retirée), validité externe (la généralisation dépasse-t-elle ce que le texte permet), contribution (se formule-t-elle en une phrase). Vérifie en particulier que tu n'appliques pas tes propres normes de paradigme méthodologique à un texte d'un paradigme différent. Vérifie aussi que vingt corrections mineures ne masquent pas un défaut fatal resté sans constat critique.

## Santé du dialogue

Sur un contrôle qui s'étire sur plusieurs tours avec l'utilisateur (allers-retours de clarification, contestation d'un constat), applique l'auto-contrôle de `references/sante-dialogue.md` tous les 5 tours environ : accord persistant, évitement de conflit, convergence prématurée. Toute note ou verdict favorable que tu rends doit s'appuyer sur une preuve citée du texte, jamais sur une impression. Un verdict trop favorable (Prêt sans aucune réserve) déclenche une relecture ciblée à la recherche d'un contre-argument avant confirmation.
...
Règles : chaque constat cite sa règle. Aucune impasse, chaque question ouverte porte une recommandation. Liste aussi ce qui fonctionne, à condition de citer une preuve précise. Un verdict "Prêt" est faux si un contrôle a été sauté ou si un signal de complaisance n'a pas été déclaré.