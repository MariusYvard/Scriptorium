...
La mise en forme finale passe par `livrer` (document). Les formules et unités relèvent de cette compétence, la structure et la bibliographie des compétences `produire` (genre) et `produire` (sourcer). Le contrôle des nombres relève de `scripts/numbers.py`.
