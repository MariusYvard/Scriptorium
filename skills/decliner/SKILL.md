...
name: decliner
description: >
  Décline un document validé en plusieurs livrables adaptés à leur canal (présentation, résumé d'une page, abstract, post professionnel, communiqué), en gardant les mêmes faits et la même charte. À utiliser quand l'utilisateur demande "décline ce rapport", "version courte", "résumé exécutif", "fais-en un deck", "un abstract", "un post LinkedIn", "un communiqué" ou veut tirer plusieurs formats d'un même fond.
metadata:
  version: "0.1.0"
---
...
Ne décliner qu'un document déjà révisé. La déclinaison propage le contenu, elle ne le corrige pas. Si le source n'est pas validé, lancer `reviser` d'abord.
...
Appliquer la même charte graphique (voir `charte-graphique`) à tous les livrables visuels, et le style maison à tous les textes. Repasser chaque déclinaison par `scripts/lint-style.py` et le scorecard.
