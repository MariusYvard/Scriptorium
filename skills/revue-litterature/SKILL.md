...
description: >
  Synthétise plusieurs sources en une revue unique : tableau de preuves dédupliqué avec attribution par source, schéma de sélection de type PRISMA, et synthèse thématique. À utiliser quand l'utilisateur demande "revue de littérature", "synthétise ces articles", "état de l'art", "revue systématique", "PRISMA" ou fournit un corpus de plusieurs documents à fondre en une analyse.
metadata:
  version: "0.1.0"
---
...
Pour chaque source retenue, déléguer à l'agent `synthese-sources` l'extraction des faits, chiffres et conclusions, chacun daté et attribué. Pondérer par fiabilité et récence (voir `sourcer`).
...
4. La bibliographie formatée (voir `sourcer`).
